 ...
 */
ChatHandler.prototype.Initialize = function( consoleObj ) {
    this.gameConsole = consoleObj;
...
ChatHandler.prototype.postMsg = function(dest, msg) {
    ...
    this.gameConsole.innerHTML += "&nbsp;" + game.player.name + ": " + decodeURIComponent(msg) + "<br>";
    this.gameConsole.scrollTop += 20;
};
...
ChatHandler.prototype.showUpdates = function() {
    if(chat_msgRequester.httpReq.readyState == 4) {
        ...
        this.updateID = window.setTimeout(gameConsole.chatHandler.update, this.UPDATE_INTERVAL);
    }
};
...
ChatHandler.prototype.update = function() {  
    if(game && game.player)
        chat_msgRequester.sendRequest(gameConsole.chatHandler.showUpdates, "GET", "chat?time=" + chat_lastUpdate + "&user=" + game.player.name, true);
};
